...
function xmldb_block_accessibility_upgrade($oldversion = 0) {
    ...
    if ($oldversion < 2023041201) {
        ...
        // Define field autoload_atbar to be dropped from block_accessibility.
        $table = new xmldb_table('block_accessibility');
        $field = new xmldb_field('autoload_atbar');

        // Conditionally launch drop field autoload_atbar.
        if ($dbman->field_exists($table, $field)) {
            $dbman->drop_field($table, $field);
        }

        // Accessibility savepoint reached.
        upgrade_block_savepoint(true, 2023041201, 'accessibility');
    }
